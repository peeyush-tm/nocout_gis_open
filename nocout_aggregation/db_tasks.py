...
db_tasks.py
============
Defines tasks related to db ops
"""
...
@app.task(base=DatabaseTask, name='mysql_export-new', ignore_result=True, time_limit=60*4, bind=True)
...
@app.task(base=DatabaseTask, name='get_inventory_devices-new')
def get_active_inventory_devices(machine=None, all=True):
	...
	for device_set in izip_longest(*[iter(devices)] * DEVICE_SET, fillvalue=''):
		yield list(chain.from_iterable(device_set))
	print '### Completed ###'


@app.task(base=DatabaseTask, name='mysql_read-new', time_limit=60*8, bind=True)
