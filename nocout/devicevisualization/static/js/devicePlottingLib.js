...
function devicePlottingClass_gmap() {
    ...
    this.showStateWiseData_gmap = function(dataset) {
        ...
        for(var i=dataset.length;i--;) {
            ...
            /*Create BS state, city, Region object*/
            if(state) {
            ...
            for(var y=0;y<bs_sectors.length;y++) {
                ...
                if(technology_filter.length > 0 || vendor_filter.length > 0 || frequency_filter.length > 0 || polarization_filter.length > 0 || antena_type_filter.length > 0) {
                    var advance_filter_condition1 = technology_filter.length > 0 ? technology_filter.indexOf(sector_technology) > -1 : true,
                        ...
                        advance_filter_condition5 = antena_type_filter.length > 0 ? antena_type_filter.indexOf(sector_antena_type) > -1 : true;

                    if(advance_filter_condition1 && advance_filter_condition2 && advance_filter_condition3 && advance_filter_condition4 && advance_filter_condition5) {
    ...
    this.getFilteredData_gmap = function() {
        ...
        var complete_filtering_data = gmap_self.getSelectedFilteringItems(),
            ...
            polarization_filter = complete_filtering_data["advance"]["polarization"],
            antena_type_filter = complete_filtering_data["advance"]["antena_type"],
            region_filter = complete_filtering_data["advance"]["region"],
            filterObj = complete_filtering_data["basic"],
            isAdvanceFilterApplied = technology_filter.length > 0 || vendor_filter.length > 0 || state_filter.length > 0 || city_filter.length > 0 || frequency_filter.length > 0 || polarization_filter.length > 0 || antena_type_filter.length > 0 || region_filter.length > 0,
            isBasicFilterApplied = $.trim($("#technology").val()).length > 0 || $.trim($("#vendor").val()).length > 0 || $.trim($("#state").val()).length > 0 || $.trim($("#city").val()).length > 0 || $.trim($("#region").val()).length > 0;
        ...
        var filtered_Data = all_devices_loki_db.where(function(obj) {
            ...
            var bs_city = obj.city ? $.trim(obj.city.toLowerCase()) : "",
                ...
                region = obj.region ? $.trim(obj.region.toLowerCase()) : "",
                sectors = obj.sectors,
                ...
                basic_filter_condition5 = filterObj['region'] != 'Select Region' ? region == $.trim(filterObj['region'].toLowerCase()) : true,
                ...
                advance_filter_condition8 =  region_filter.length > 0 ?  region_filter.indexOf(region) > -1 : true;
            ...
            if(isBasicFilterApplied) {
                ...
                if($.trim($("#technology").val()).length > 0 || $.trim($("#vendor").val()).length > 0) {
                            ...
                            if(
                                ...
                                if(isAdvanceFilterApplied) {
                                    ...
                                    var sector_frequency_1 = sectors[i].freq ? $.trim(sectors[i].freq) : "",
                                        ...
                                        advance_filter_condition7 = antena_type_filter.length > 0 ? antena_type_filter.indexOf(sector_antena_type) > -1 : true;
                                    ...
                                    if(
                                        ...
                                        &&
                                        advance_filter_condition7
                                        &&
                                        advance_filter_condition8
                                    ) {
                                        isCorrect = true;
                                        break;
                                    }
                    ...
                    if(basic_filter_condition1 && basic_filter_condition2 && basic_filter_condition5) {
                        var isCorrect = false;
                        // If advance Filters Applied
                        if(isAdvanceFilterApplied) {
                            if(!isCorrect) {
    ...
    this.getBasicFilters = function() {
        ...
        /*Populate Region*/
        var region_option = "";
        region_option = "<option value=''>Select Region</option>";

        for(var i=all_region_array.length;i--;) {
            region_option += "<option value='"+i+1+"'>"+all_region_array[i]+"</option>";
        }
    ...
    this.loadAdvanceFilters = function() {
                    ...
                    for(var j=0;j<antenna_type_list.length;j++) {
                        if(searchPattern.test(antenna_type_list[j])) {
                            if(data.results.length >= 40) {
                                ...
                                if(antenna_type_array.indexOf(antenna_type_list[j].toLowerCase()) < 0) {
                                    antenna_type_array.push(antenna_type_list[j].toLowerCase());
        ...
        $("#filter_region").select2({
            ...
            query: function (query) {
                ...
                var filtered_data = all_devices_loki_db.where(function(obj) {
                    var searchPattern = new RegExp('^' + query.term, 'i');
                    if(searchPattern.test(obj.region)) {
                        return true;
                    } else {
                        return false;
                    }
                ...
                var limit = filtered_data.length <= 40 ? filtered_data.length : 40;
                for (i = 0; i < filtered_data.length; i++) {
                    if(showing_regions.indexOf(filtered_data[i].region) < 0) {
                        if(data.results.length >= 40) {
                            break;
                        } else {
                            showing_regions.push(filtered_data[i].region);
                            data.results.push({
                                id: filtered_data[i].region,
                                text: filtered_data[i].region,
                                value : filtered_data[i].region
                            });
                        }
                    }
                }
    ...
    this.getSelectedFilteringItems = function() {
        ...
        var has_technology = $("#filter_technology").select2('val').length > 0,
            ...
            has_antena_type = $("#filter_antena_type").length > 0 ? $("#filter_antena_type").select2('val').length > 0 : false,
            has_region = $("#filter_region").length > 0 ? $("#filter_region").select2('val').length > 0 : false,
            ...
            antenna_type_data = has_antena_type ? $("#filter_antena_type").select2('data') : [],
            region_data = has_region ? $("#filter_region").select2('data') : [],
            technology_filter = [],
            vendor_filter = [],
            city_filter = [],
            state_filter = [],
            ...
            selected_filters_data = {
                "basic" : {
                    ...
                    "region" : $.trim($("#region option:selected").text())
                },
                "advance" : {
                    "technology" : [],
                    "vendor" : [],
        ...
        // If any region filter is applied the get selected regions
        if(has_region && region_data.length > 0) {
            region_filter = gmap_self.prepareSelect2Array(region_data);
        }
        ...
        selected_filters_data["advance"]["region"] = region_filter;
    ...
    this.makeFiltersArray = function(mapPageType) {
        ...
        if($("#region").val().length > 0) {
            appliedFilterObj_gmaps["region"] = $.trim($("#region option:selected").text());
        }
