...
//			var resultantObject = {};
			...
			if(elementType == "multiselect") {
				...
				if(val.length > 0){
                    ...
                    {
                        if( val[k].indexOf(',')>=0){
                          appliedAdvFilter=appliedAdvFilter.concat(val[k].split(','))
                        }
                        else{
                            appliedAdvFilter.push(val[k])
                         }
                    }

                appliedAdvFilter_Active.push({'field':selectId, 'value':val});
                }
//                appliedAdvFilter= appliedAdvFilter.concat(val);
            }
            console.log(appliedAdvFilter)
...
//			}
		...
		advSearch_self.setFilters(searchParameters);
	...
	this.setFilters = function(searchString) {
        ...
        loop2:
                for(var j= 0;main_devices_data_gmaps.length>j;j++){
                    ...
                    result_plot_device.push(main_devices_data_gmaps[j])
                    break loop2
                    }
                }
            }
...
//            }
            ...
            /*Call the make network to create the BS-SS network on the google map*/
            gmapInstance.plotDevices_gmap(result_plot_device,"base_station");
            /************** GOOGLE EARTH CODE*********************/
