...
;(function($) {
        ...
        var options = $.extend({}, $.fn.formset.defaults, opts),
            ...
            insertDeleteLink = function(row) {
                if (row.is('TR')) {
                    ...
                    row.children(':last').append('<a class="' + options.deleteCssClass + ' ' + options.buttonCssClass + '" href="javascript:void(0)">' + options.deleteText + '</a>');
                } else if (row.is('UL') || row.is('OL')) {
                    // If they're laid out as an ordered/unordered list,
                    // insert an <li> after the last list item:
                    row.append('<li><a class="' + options.deleteCssClass + ' ' + options.buttonCssClass + '" href="javascript:void(0)">' + options.deleteText +'</a></li>');
                } else {
                    // Otherwise, just insert the remove button as the
                    // last child element of the form's container:
                    row = row.find('.form-group').eq(0).find('.col-sm-9')
                    row.append('<a class="' + options.deleteCssClass + ' ' + options.buttonCssClass + '" href="javascript:void(0)">' + options.deleteText +'</a>');
                }
    ...
    $.fn.formset.defaults = {
        ...
        addCssClass: 'add-row btn button',          // CSS class applied to the add link
