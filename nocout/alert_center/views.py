                    ...
                    if len(substation):
                        ...
                        if data.severity in ['DOWN', 'CRITICAL', 'WARNING', 'UNKNOWN'] or \
                            ...
                            device_events = {
                                ...
                                'sys_time': datetime.datetime.fromtimestamp(float(data.sys_timestamp)).strftime(
                                    "%I:%M:%S %p"),
                                'sys_date': datetime.datetime.fromtimestamp(float(data.sys_timestamp)).strftime("%d/%B/%Y"),
                                'sys_timestamp': datetime.datetime.fromtimestamp(float(data.sys_timestamp)).strftime(
                                    "%m/%d/%y (%b) %H:%M:%S (%I:%M %p)"),
    ...
    def get_initial_queryset(self):
        ...
        for machine, machine_device_list in machine_dict.items():
            ...
            query_table_eventnetwork = prepare_query(table_name="performance_eventnetwork", devices=machine_device_list, \
                                                     ...
                                                     columns=required_data_columns)  #$, condition=extra_query_condition)
                    ...
                    if len(sector):
                        ...
                        if data.severity in ['DOWN', 'CRITICAL', 'WARNING', 'UNKNOWN'] or \
                            ...
                            ddata = {
                                ...
                                'sys_time': datetime.datetime.fromtimestamp(float(data.sys_timestamp)).strftime(
                                    "%I:%M:%S %p"),
                                'sys_date': datetime.datetime.fromtimestamp(float(data.sys_timestamp)).strftime(
                                    "%d/%B/%Y"),
                                'sys_timestamp': datetime.datetime.fromtimestamp(float(data.sys_timestamp)).strftime(
                                    "%m/%d/%y (%b) %H:%M:%S (%I:%M %p)"),
                                'description': data.description
        ...
        if qs:
            ...
            for dct in qs:
                ...
                dct.update(action='<a href="/alert_center/customer/device/{0}/service_tab/{1}/"><i class="fa fa-warning text-warning"></i></a>\
                                   ...
                                   <a href="/device/{0}"><i class="fa fa-dropbox text-muted"></i></a>'.format(device.id,
                                                                                                              service_tab,
                                                                                                              device.substation_set.values()[
                                                                                                                  0][
                                                                                                                  'id']))
