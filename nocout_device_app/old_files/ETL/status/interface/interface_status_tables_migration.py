"""
interface_status_tables_migrations.py
=====================================
...
# Get logger
logger = logging_module.get_site_logger('interface_status_migrations.log')
...
def main(**configs):
    ...
    pformat('[-- main --] start_time, end_time: ' + pformat(start_time) + ' ' + pformat(end_time))
...
def insert_data(table, data_values, **kwargs):
	...
	db = utility_module.mysql_conn(configs=kwargs.get('configs'))
		...
		query = "SELECT * FROM %s " % table +\
                "WHERE `device_name`='%s' AND `site_name`='%s' AND `service_name`='%s' AND `data_source`='%s'" %(str(data_values[i][0]),data_values[i][3],data_values[i][1],data_values[i][4])
        	cursor = db.cursor()
        	try:
