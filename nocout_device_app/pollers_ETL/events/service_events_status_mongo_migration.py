"""
service_events_status_mongo_migration.py
========================================

Script to bulk insert current events status data from
pollers to mysql in 5 min interval for all services.

Current event status data means for each (host, service, severity), only most latest entry would
be kept in the database, which describes the event for that service running on a particular host,
at any given time.
...
def read_data(start_time, end_time, **kwargs):
    ...
    if db:
        if start_time is None:
            cur = db.service_event_status.find()
        else:
            cur = db.service_event_status.find({
...
def insert_data(table, data_values,**kwargs):
    ...
    if len(insert_dict['1']):
        ...
        query += """SET `site_name`=%s, `machine_name`=%s,
        `warning_threshold`=%s, `critical_threshold`=%s, `sys_timestamp`=%s,
        `check_timestamp`=%s, `ip_address`=%s,`severity`=%s, `description`=%s
        WHERE `device_name`=%s AND `service_name`=%s
        """
        try:
            data_values = map(lambda x: ( x[3], x[2], x[9], x[10], x[11], x[12], 
                x[13], x[14], x[15]) + (x[0], x[1]), insert_dict.get('1'))
