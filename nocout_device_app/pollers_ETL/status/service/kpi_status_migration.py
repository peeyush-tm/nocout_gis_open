...
def main():
    ...
    kpi_status_tables_script = configs.get(mongo_conf[0][0]).get('kpi_status_tables').get('script')
    kpi_status_tables_migration_script = __import__(kpi_status_tables_script)
    kpi_status_tables_migration_script.main(mongo_conf=mongo_conf,
    ...
    sql_db=configs.get(mongo_conf[0][0]).get('sql_db'), table_name=configs.get(mongo_conf[0][0]).get('kpi_status_tables').get('table_name'), ip=configs.get(mongo_conf[0][0]).get('ip')
    )
