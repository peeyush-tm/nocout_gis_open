...
class correlation:
	...
	# If switch/conv is Down: Do not generate any Traps.
	#Cases Handled here:
	#1.) SiteA down
	...
	if sitea_conv_switch_list:
	    logger.error("SiteA switch conv is down")
	    return None,None
	else:
	...
	"""
	Alarm filteration for each faulty device based on priority given in mat table.
	Returns None if Clear Alarm is present.
	"""
...
@app.task(baset=DatabaseTask, name='clear_trap')
def clear_trap(keys):
    """
    ...
    p.execute()
    map(lambda trap: trap.update({'severity':'clear'}) ,clear_traps)
...
def correlation_down_event():  
	...
	#changes for Correlation Sprint2 Requirement
...
def send_traps(params):
	...
	if ptp_bh_flag:
	    ...
	    # Trap for SiteB ptp backhaul device.
    ...
    [p.hset('correlated_traps', str(trap_dict.get('resource_name')+'_'+trap_dict['alrm_name']), trap_dict) for trap_dict in non_ckt_trap_list]
    p.execute()
