...
class Singleton(type):
    ...
    def __call__(cls, *args, **kwargs):
	    ...
	    #TODO: mat temporary on 6
	    rds_cli = RedisInterface(custom_conf={'db': 6})
...
class correlation:
	...
	for trap_event in trap_event_list:
	    ...
	    alarm['timestamp'] = datetime.strptime(trap_event[8],"%Y-%m-%d %H:%M:%S").strftime('%s')
	...
	correlation_down_event.apply_async(queue='correlation')

	# TODO:Modify code for sending clear trap.
	# Clear trap with id sent by the monolith for correlated trap. 
	"""   
	...
	alarm_name= down_device_dict.get(key).get(conv_switch_mapping_dict['alrm_name'],'')
	...
	conv_switch_list= [element_dict.get(index) for index in range(3,6) if element_dict.get(index)]
...
@app.task(base=DatabaseTask, name='correlation_update_inventory',queue='correlation')
...
@app.task(base=DatabaseTask, name='correlation_down_event',queue='correlation')
def correlation_down_event():  
    ...
    invent_ids = filter(lambda x:int(current_time) - int(invent_dict[x]['timestamp']) >420  and 
		 int(current_time) - int(invent_dict[x]['timestamp']) <= 600 , invent_dict.keys())
    ...
    update_invent_dict.s(invent_ids).apply_async(queue='correlation') 
	...
	if del_ss:
        ...
        send_traps.s(params).apply_async(queue='correlation')

@app.task(base=DatabaseTask, name='send_traps',queue='correlation')
def send_traps(params):
    ...
    final_trap_list = sitea_trap_list + siteb_trap_list

    """
    trap_sender_task.s(final_trap_list).apply_async()
    
    # Store the non circuit trap with the id generated by monolith.
    non_ckt_trap_dict = {}
...
@app.task(base=DatabaseTask, name='trap_sender_task',queue='correlation')
...
@app.task(base=DatabaseTask, name='update_invent_dict',queue='correlation')
