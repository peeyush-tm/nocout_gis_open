...
def main():
    ...
    #print mongo_conf	
    radwin5k_topology_script = configs.get(mongo_conf[0][0]).get('Radwin5k_topology').get('script')
    #print radwin5k_topology_script
    radwin5k_topology_migration_script = __import__(radwin5k_topology_script)
    #print radwin5k_topology_migration_script
    radwin5k_topology_migration_script.main(mongo_conf=mongo_conf,
    ...
    sql_db=configs.get(mongo_conf[0][0]).get('historical_db'), table_name=configs.get(mongo_conf[0][0]).get('Radwin5k_topology').get('table_name'), ip=configs.get(mongo_conf[0][0]).get('historical_ip')
    )
