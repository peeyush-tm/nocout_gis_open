...
def poll_device(**kw):
	...
	try:
		...
		device_list = literal_eval(kw.get('device_list'))
		service_list = literal_eval(kw.get('service_list'))
		bs_name_ss_mac_mapping = literal_eval(kw.get('bs_name_ss_mac_mapping'))
		ss_name_mac_mapping = literal_eval(kw.get('ss_name_mac_mapping'))
	...
	try:
		data_source_list = literal_eval(kw.get('ds'))
	except Exception as exc:
		data_source_list = ['']
...
def get_current_value(
	...
	for service in service_list:
		...
		logger.debug(' Check_output: {0}'.format(check_output))
		if check_output:
